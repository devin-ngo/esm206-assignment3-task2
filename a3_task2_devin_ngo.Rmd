...
sw_split_name <- starwars %>% # Start with starwars
  separate(col = name, # Separate the name column
           ...
           sep = "-" # Separated at the first '-'
  )

# In row 49, the third part of the name after the second dash was omitted
```
...
sw_skin_sep <- starwars %>% # Start from starwars data 
  separate(col = skin_color, # Separate the 'skin_color' column
           ...
           sep = ",", # Creating separation at the first ','
           extra = "merge", # If additional ',', merge all into second column
           remove = FALSE) # Keep original 'skin_color' column
```
...
mpg_model_class <- mpg %>% # Start with mpg
  unite(col = model_class, # Create new column 'model_class'
        c(model, class), # That combines model and class
        sep = "-", # Puts a '-' between the pieces
...
gasol_hookshots <- lakers %>% 
  filter(player %in% c("Pau Gasol"), type %in% c("hook")) %>% # Filter for Pau Gasol and 'hook' shot
  mutate(date_new = ymd(date)) %>% # Updating column to 'Date' class
  mutate(month_new = month(date_new)) # Adding month column

gasol_hookshots %>% 
  count(month_new) # Finding number of Gasol's hook shots by month
```
